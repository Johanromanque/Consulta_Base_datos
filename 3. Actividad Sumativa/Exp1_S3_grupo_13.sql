...
SELECT
-- formatea el rut con puntos de miles y agrega el digito verificador
  TO_CHAR(c.numrut_cli,'FM999G999G999','NLS_NUMERIC_CHARACTERS=,.')
    || '-' || c.dvrut_cli                               AS "RUT Cliente",
    
-- construye el nombre completo y lo capitaliza (primera letra)
  INITCAP(c.nombre_cli || ' ' || c.appaterno_cli || ' ' || c.apmaterno_cli)
                                                        AS "Nombre Completo Cliente",
                                                        
-- direccion del cliente con formato de capitalizacion                                                        
  INITCAP(c.direccion_cli)                              AS "Dirección Cliente",
  
-- renta formateada con separador de miles y simbolo $  
  '$' || TO_CHAR(c.renta_cli,'FM9G999G999','NLS_NUMERIC_CHARACTERS=,.')
                                                        AS "Renta Cliente",
                                                        
-- si el cliente no tiene celular, mostrar "sin celular"                                                        
  NVL(TO_CHAR(c.celular_cli), 'Sin Celular')            AS "Celular Cliente",
  
-- clasificacion del cliennte por nivel de renta  
  CASE
...
-- se muestra solo clientes con celular registrados
WHERE c.celular_cli IS NOT NULL

-- filtra por rango de renta ingresado por usuario
-- se usa REPLACE para permitir valores introducidos con puntos 
  AND c.renta_cli BETWEEN
        TO_NUMBER(REPLACE('&RENTA_MINIMA','.',''))
    AND TO_NUMBER(REPLACE('&RENTA_MAXIMA','.',''))
    
-- ordena el resultado por nombre del cliente     
ORDER BY INITCAP(c.nombre_cli), INITCAP(c.appaterno_cli), INITCAP(c.apmaterno_cli);
...
-- promedio de sueldos formateado en pesos  
  '$' || TO_CHAR(ROUND(AVG(e.sueldo_emp)),
         'FM9G999G999','NLS_NUMERIC_CHARACTERS=,.') AS "SUELDO_PROMEDIO"
FROM empleado e, categoria_empleado ce, sucursal s

-- relaciones entre tablas 
WHERE e.id_categoria_emp = ce.id_categoria_emp
  AND e.id_sucursal      = s.id_sucursal
  
-- agrupa los resultados por categoria y sucursal  
GROUP BY e.id_categoria_emp, ce.desc_categoria_emp, s.desc_sucursal

-- filtra para mostrar solo categorias cuyo sueldo promedio es mayor al ingresado
HAVING AVG(e.sueldo_emp) > &SUELDO_PROMEDIO_MINIMO

-- ordena de mayor a menor sueldo promedio
...
-- promedio del valor de arriendo formateado en pesos  
  '$' || TO_CHAR(ROUND(AVG(p.valor_arriendo)),
         'FM9G999G999','NLS_NUMERIC_CHARACTERS=,.')                 AS "PROMEDIO_ARRIENDO",
         
-- promedio de superficie en m2         
  ROUND(AVG(p.superficie), 2)                                       AS "PROMEDIO_SUPERFICIE",
  
-- promedio del valor por m2 formateado en pesos
  '$' || TO_CHAR(ROUND(AVG(p.valor_arriendo / p.superficie)),
         'FM9G999G999','NLS_NUMERIC_CHARACTERS=,.')                 AS "VALOR_ARRIENDO_M2",
         
-- clasificacion segun valor por m2         
  CASE
...
FROM propiedad p, tipo_propiedad tp

-- relacion entre propiedad y su tipo
WHERE p.id_tipo_propiedad = tp.id_tipo_propiedad

-- agrupa pot tipo de propiedad
GROUP BY tp.id_tipo_propiedad, tp.desc_tipo_propiedad

-- filtra para mostrar solo propiedades cuyo valor por m2 es mayor a 1000
HAVING AVG(p.valor_arriendo / p.superficie) > 1000

-- ordena de mayor a menor valorpor m2
